...
export const customerUser = defineType({
  name: "user", // Keep same name for backward compatibility
  title: "Customer User",
  type: "document",
  readOnly: true,
  description:
    "Customer users for KAPCDAM e-commerce platform. Authentication handled by Clerk. These users can place orders and have carts.",
    ...
    rule.custom(async (document, context) => {
      ...
      if (document.clerkUserId) {
        ...
        if (existingClerkUsers > 0) {
          errors.push(
            `An active customer user with Clerk ID "${document.clerkUserId}" already exists.`
          );
        }

        // Check if this Clerk ID exists as an admin team member
        const existingAdminTeamMember = await client.fetch(
          `count(*[_type == "team" && clerkId == $clerkUserId && role == "admin" && isActive == true])`,
          {
            clerkUserId: document.clerkUserId,
          }
        );

        if (existingAdminTeamMember > 0) {
          errors.push(
            `This Clerk ID "${document.clerkUserId}" is already registered as an admin user. Admin and customer users must be separate.`
          );
        }
      }
      ...
      if (document.email) {
        ...
        if (existingEmailUsers > 0) {
          errors.push(
            `An active customer user with email "${document.email}" already exists.`
          );
        }
      }
