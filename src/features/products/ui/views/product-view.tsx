...
import { Star, Heart, Minus, Plus, Share2, Send, ThumbsUp, ThumbsDown } from "lucide-react";
...
export default function ProductView({ slug }: ProductViewProps) {
  ...
  const handleVoteOnReview = (reviewId: string, voteType: "helpful" | "not_helpful") => {
    if (!isSignedIn) {
  ...
  return (
    <div className="max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8">
      ...
      <div className="space-y-8">
        {/* 1. Product Title & Basic Info - Always First on Mobile */}
            ...
            {currentImages.length > 1 && (
              <div className="flex gap-2 overflow-x-auto pb-2">
                {currentImages.slice(0, 6).map((image, i) => (
                  <div
                    key={i}
                    className={`flex-shrink-0 w-16 h-16 relative border-2 rounded transition-colors cursor-pointer ${
                      i === currentImageIndex
                        ? "border-primary"
                ...
                {currentImages.length > 6 && (
                  <button
                    className="flex-shrink-0 w-16 h-16 bg-muted flex items-center justify-center text-xs font-medium hover:bg-muted/80 transition-colors rounded"
      ...
      {/* 4. Reviews Section - Always at Bottom */}
      <div className="mt-12 space-y-6">
        ...
        {/* Add Review Form */}
        {isSignedIn ? (
          <div className="space-y-4 p-4 sm:p-6 bg-muted/50 border border-border rounded-lg">
          ...
          <div className="p-4 sm:p-6 bg-muted/50 border border-border text-center rounded-lg">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      disabled={!!userVotes[review._id] || voteOnReviewMutation.isPending}
                      className={`h-8 px-2 text-xs ${
                        userVotes[review._id] === "helpful" ? "bg-green-50 text-green-700" : ""
                      }`}
                    >
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => handleVoteOnReview(review._id, "not_helpful")}
                      disabled={!!userVotes[review._id] || voteOnReviewMutation.isPending}
                      className={`h-8 px-2 text-xs ${
                        userVotes[review._id] === "not_helpful" ? "bg-red-50 text-red-700" : ""
                      }`}
                    >
